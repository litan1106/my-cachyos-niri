...
Rectangle {
  ...
  ColumnLayout {
    ...
    // Bottom Bar (Windows 11 / Windows 10 style)
    ColumnLayout {
      Layout.fillWidth: true
      Layout.leftMargin: Style.marginL
      Layout.rightMargin: Style.marginL
      ...
      RowLayout {
        ...
        // Left Side: User profile avatar and name
        RowLayout {
          ...
          Rectangle {
            ...
            Image {
              ...
              anchors.fill: parent
              source: {
                var av = Settings.data.general.avatarImage;
                if (av && av !== "") {
                  return "file://" + av;
                }
                return "";
        ...
        NIconButton {
          id: powerButton
          icon: "power"
          tooltipText: I18n.tr("tooltips.session-menu") || "Power"
          customRadius: Style.iRadiusM
          Layout.preferredWidth: 32
          Layout.preferredHeight: 32
          Layout.alignment: Qt.AlignRight | Qt.AlignVCenter
          onClicked: {
            powerMenu.openAtItem(powerButton, root.screen);
          }
        }
      }
    }
  ...
  // Windows-style Start Menu Power Menu
  NPopupContextMenu {
    id: powerMenu
    screen: root.screen
    model: [
      {
        "label": I18n.tr("common.lock") || "Lock",
        "action": "lock",
        "icon": "lock"
      },
      {
        "label": I18n.tr("common.suspend") || "Sleep",
        "action": "suspend",
        "icon": "moon"
      },
      {
        "label": I18n.tr("common.reboot") || "Restart",
        "action": "reboot",
        "icon": "rotate"
      },
      {
        "label": I18n.tr("common.shutdown") || "Shut Down",
        "action": "shutdown",
        "icon": "power"
      }
    ]
    onTriggered: (action, item) => {
      close();
      root.close(); // Close the launcher panel
